...
HANDOUT_DEFAULT_SEASON_INDEX = 0  # newest League Two season from meta (26/27)
...
def resolve_opponent_kit(opponent: dict[str, Any] | None) -> dict[str, str]:
    """Public kit resolver for set-piece / handout theming."""
    return _resolve_opponent_kit(opponent or {})
...
def _resolve_opponent_kit(opponent: dict[str, Any]) -> dict[str, str]:
    ...
    for key, kit in OPPONENT_KITS_BY_KEY.items():
        if key in name:
            merged = {**DEFAULT_OPPONENT_KIT, **kit}
            if not merged.get("secondary"):
                # Light text kits get a dark secondary strip; dark kits get white.
                merged["secondary"] = (
                    "#111111" if merged.get("text", "#111").casefold() in {"#111111", "#111", "#000000", "#000"} else "#FFFFFF"
                )
            return merged
    return {**dict(DEFAULT_OPPONENT_KIT), "secondary": "#F5C518"}
...
def _pick_handout_iteration(iterations: list[dict[str, Any]]) -> dict[str, Any]:
    """Always prefer the newest season from meta (index 0 after reverse season sort)."""
...
def _default_handout_fixture(iteration_id: int) -> dict[str, Any] | None:
    """Port Vale's next fixture (soonest future kickoff)."""
...
def register_pre_match_handout_routes(app: FastAPI) -> None:
    ...
    def pre_match_handout_page() -> HTMLResponse:
        # Canonical URL is /pre-match — keep this path as a bookmark alias.
